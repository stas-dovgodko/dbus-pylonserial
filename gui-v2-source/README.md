...
device entry. The D-Bus service uses the isolated `telemetry` device type, so it is
...
appears in Device List as a telemetry device and its `/Reason` field shows
